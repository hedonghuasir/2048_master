...
@interface hdhGomokuChessModelTemplate ()
@end

@implementation hdhGomokuChessModelTemplate


+(instancetype)shareTemplete
{
    static hdhGomokuChessModelTemplate * templete=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        templete=[[hdhGomokuChessModelTemplate alloc]init];
    });
    return templete;
}
...
-(NSInteger)calculateScoreWithModelValue:(long long)modelValue
{
  ...
  //  printf("First :%d   Second:%d\n",maxFirstValue,maxSecondValue);
    NSInteger score;
    if ([[hdhGomokuGameEngine game].envisionStack depth]%2==0) {
...
-(NSInteger)calculateFirst:(long long)modelValue
{
    ...
    if ([hdhGomokuGameEngine game].playerFirst&&[[hdhGomokuGameEngine game].envisionStack depth]%2==0) {
